...
``` r
retail <- read_excel("data/260909s5.xlsx", 
                     sheet = "レギュラー",
...
"2026-03-10", 130.0, # 03-16(Mon) 129.90 * 159.07 / 159
"2026-03-17", 131.5, # 03-23(Mon) 131.97 * 158.44 / 159
...
"2026-08-11", 87.3, # 08-17(Mon) 87.06 * 159.45 / 159
"2026-08-18", 90.2, # 08-24(Mon) 90.10 * 159.11 / 159
"2026-08-25", 89.2, # 08-31(Mon) 88.75 * 159.74 / 159
"2026-09-01", 97.2 # 09-07(Mon) 100.16 * 154.37 / 159
) |> 
...
``` r
retail_weekly <- retail_weekly |> 
  mutate(
    consumption_tax_rate = case_when(
      week < yearweek("2004-04-01") ~ 0,
      week < yearweek("2014-04-01") ~ .05,
      week < yearweek("2019-10-01") ~ .08,
      .default = .1),
...
combo_weekly <- retail_weekly |> 
  left_join(meti_dubai_weekly, by = "week") |> 
...
``` r
wholesale <- read_excel("data/260831o5.xlsx",
                        sheet = "レギュラー",
