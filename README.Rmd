...
wholesale <- read_excel("data/250530o5.xlsx",
                        sheet = "レギュラー",
...
retail <- read_excel("data/250730s5.xlsx", 
                     sheet = "レギュラー",
...
retail <- retail |> 
  bind_rows(tibble(date = as.Date("2025-05-05"), price = 184.5)) |> 
  arrange(date)
...
METI stopped publishing Dubai crude oil prices on June 11, 2025. They were calculated weekly average yen per litre from Tuesday to Monday. Since then, I use [dollar per barrel](https://www.investing.com/commodities/dubai-crude-oil-platts-futures-historical-data), [yen per dollar](https://www.investing.com/currencies/usd-jpy-historical-data), and 159 litre per barrel to calculate yen per litre only on Monday
...
"2025-07-22", 66.0 # 07-28(Mon) 65.2 = 70.59 * 148.55 / 159
) %>% 
...
retail |> 
  ...
  mutate(
    ...
    retail_meti_new = retail_actual + lag(subsidy, 2) + lag(crude_oil) - lag(crude_oil, 2),
    retail_meti_new = if_else(week >= make_yearweek(2025, 22),
                              retail_actual + lag(subsidy, 2),
                              retail_meti_new),
