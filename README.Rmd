...
retail <- read_excel("data/230215s5.xls", 
                     sheet = "レギュラー",
...
combo %>% 
  filter(name == "diff_rw", !is.na(value)) %>% 
  mutate(diff_from_mean = value - mean_diff_2021) %>% 
  select(month, diff_from_mean) %>% 
  filter(month >= yearmonth("2022 Feb")) %>% 
  knitr::kable(format = "simple",
               digits = 1,
               col.names = c("Month", "Excess over 2021 average"),
               caption = "Retailers' excess margin (yen / liter)")
```

The average retailers' margin in 2021 was
`r mean_diff_2021 %>% round(digits = 1)`. If I subtract it from retailers' margin, I get excess margin above.


Differences between wholesale price and imported crude oil price plus gasoline tax have also been increasing. The average difference was `r combo %>% filter(name == "diff_wc") %>% filter_index("2001 Jan" ~ "2014 Dec") %>% pull(value) %>% mean() %>% round()` yen per litre from 2001 Jan to 2014 Dec, and is `r combo %>% filter(name == "diff_wc", !is.na(value), month == yearmonth("2022 Jan")) %>%  pull(value) %>% round()` in 2022 Jan. This may reflect the reduced competition among wholesalers, who have got monopolistic power by consolidation. The decrease from 2022 Feb is due to subsidy.
...
combo %>% 
  ...
  geom_text(aes(label = month(month, label = TRUE)),
            ...
            hjust = -0.2, vjust = 0) +
  labs(y = "Yen per litre",
       title = "Wholesalers' margin = wholesale price - (imported crude oil price +\ngasoline tax)")
...
```{r dub_import_diff}
dub_import_diff <- dub_import %>% 
  mutate(
    dub_lag_1 = lag(dub_oil_price, 1),
    diff = import_price - dub_lag_1
  ) %>% 
  filter(!is.na(diff))
...
dub_import_diff %>% 
  autoplot(diff) +
  labs(y = "Yen per litre",
       title = "Import price - one month ago Dubai price")
```

Import price is higer than Dubai price by 
`r dub_import_diff %>% 
  as_tibble() %>% 
  ...
  round(digits = 2)
`
yen per litre on average, probably due to CIF costs.
...
```{r dub_import_wholesale_diff}

dub_import_wholesale_diff <- dub_import_wholesale %>% 
  mutate(
    dub_oil_price_lag = lag(dub_oil_price),
    gas_tax = 53.8,
    diff = wholesale_price - dub_oil_price_lag - gas_tax
  ) %>% 
  filter(!is.na(diff))

dub_import_wholesale_diff %>% 
  autoplot(diff) +
  geom_text(aes(label = month), 
            vjust = 0, hjust = 1,
            data = . %>% slice_tail(n = 1)) +
  geom_text(aes(label = round(diff, digits = 0)), 
            vjust = 0, hjust = 0,
            data = . %>% slice_tail(n = 1)) +
  labs(y = "Yen per litre",
       title = "Historical wholesalers' margin = wholesale price -\n(one month ago Dubai crude oil price + gas tax)")
