...
retail <- read_excel("data/220615s5.xls", 
                     sheet = "レギュラー",
...
It was `r combo %>% filter(name == "diff_wc", !is.na(value), month == yearmonth("2022 Feb")) %>%  pull(value) %>% round()` in 2022 Feb, and failed to drop as much as 5 yen per litre subsidy. It was `r combo %>% filter(name == "diff_wc", !is.na(value), month == yearmonth("2022 Mar")) %>%  pull(value) %>% round()` in 2022 Mar, and failed to drop significantly, despite 20-25 yen per litre subsidy given to wholesalers in March. It was `r combo %>% filter(name == "diff_wc", !is.na(value), month == yearmonth("2022 Apr")) %>%  pull(value) %>% round()` in 2022 Apr, and failed to drop by as much as 20-35 yen per litre subsidy given to wholesalers in April.
